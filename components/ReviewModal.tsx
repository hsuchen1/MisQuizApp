    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[80vh] flex flex-col">
        <div className="flex justify-between items-center p-4 md:p-6 border-b border-gray-200">
          <h2 className="text-xl md:text-2xl font-bold text-slate-700">回顧錯題</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-4 md:p-6 overflow-y-auto custom-scrollbar flex-grow">
          {wrongAnswers.length === 0 ? (
            <p className="text-slate-600 text-center">沒有錯題可以回顧，做得好！</p>
          ) : (
            <ul className="space-y-6">
              {wrongAnswers.map((item, index) => (
                <li key={item.id || index} className="bg-slate-50 p-4 rounded-md shadow-sm">
                  <p className="text-sm font-semibold text-slate-500 mb-1">錯題 #{index + 1}</p>
                  <p className="text-base md:text-lg font-medium text-slate-800 mb-2">
                    <span className="font-semibold">問:</span> {item.question}
                  </p>
                  <p className="text-base text-green-600 font-semibold">
                    <span className="font-semibold text-green-700">答:</span> {item.correct_answer}
                  </p>
                </li>
              ))}
            </ul>
